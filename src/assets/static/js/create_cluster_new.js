...
export function create_cluster_new(d3v6, place, csvs, clusterClick, tag, filters_all, brushHandle, data_brush, find_node, word_cloud, data_rem) {
  // place = .cluster-main
  document.querySelector(place).innerHTML = '';
  var width = 840; //SVG绘制区域的宽度
  var height = 720; //SVG绘制区域的高度
  ...
  svg.call(d3v6.zoom().scaleExtent([1, 10]).on('zoom', function (event) {
  ...
  for (let key in csvs) {
    ...
    if (label_filter !== '') {
      ...
      if (confidence_filter !== '') {
        let start = confidence_re[confidence_filter];
        let end = confidence_filter < 4 ? confidence_re[confidence_filter + 1] : 0;
        if (item['confidence'] <= start && item['confidence'] > end) {
          data.push({
            ids: md5,
            x: x + Math.random() * 2 -1,
            y: y + Math.random() * 2 -1,
  ...
  //20220305
  let outlierIndex=arr1.findIndex(item=>item==-1)
  if(outlierIndex!==-1) arr1.splice(outlierIndex,1);
  ...
  //20220305
  if(outlierIndex!==-1) sorted.splice(outlierIndex,1);
  ...
  if (convex_hull) {
    ...
    hulls
      .attr('d',function(d,i){
        ...
        // // console.log(1,hullData)
        
        /*—————————————————— 20220220 —————————————————*/
        //凸包边界扩张
        /*按顺时针/你逆时针获取凸包边界上的所有点，
        相邻的三个点可以连成两条边，作这两条边的中垂线
        分别延中垂线的方向向外（一段距离）作这两条边的平行线，记录这两条平行线的交点
        每三个相邻的点都能得到一个往外扩的交点
        以这组交点为新的凸包边界*/

        /*直接用geometric.js 返回从质心缩放多边形所产生的顶点*/
        var scaled_hull = geometric.polygonScale(hullData, 1.1);
        //return d3.line()(scaled_hull)

        /*—————————————————— 20211216 —————————————————*/
        //凸包边界平滑
        return d3v6.line().curve(d3v6.curveCatmullRomClosed.alpha(0.75))(scaled_hull);

        //https://github.com/d3/d3-shape/blob/v2.0.0/README.md#curveCatmullRomClosed
        //curveCatmullRomClosed.alpha(0.75)
        //curveBasisClosed 不太好，不能把所有点围住，但这也是个思路，通常类中距离较远的就在凸包外面了
        //curveCardinalClosed.tension(0)
        //curveNatural
  ...
  if(data_rem&&data_rem.length>0){
    ...
    for(let i =0;i<data.length;i++){
      if(data_rem.indexOf(data[i].ids)!=-1){
        ...
        rest_data.push(data[i]);
      }
    }
    let data_merge=[...rest_data,...data_rem_item]
    ...
    .attr("fill", d => {
      if (tag === 'left') {
        if (brush_data_filter.indexOf(d.ids) !== -1 || find_node == d.ids) {
          ...
          // @cz 更改推荐散点颜色
          // if (d.num <= 2000 && outlier_flag) {
          // // console.log("left2")
          //   return "#C00000";
          // } 
          if(outlier_flag && data_rem_item.findIndex(item=>d.ids === item.ids) !== -1){
            // console.log('推荐节点');
            return "#c00000";//#c00000
          }
          else {
            // var co = d["confidence"];
            // // // console.log(d);
    ...
    .attr('md5', (d) => d.ids)
    .on('click', (d) => { console.log("id: ",d.ids) })
    ...
    .attr("stroke", function (d) {
      if (d.num <= 5 && outlier_flag) {
        ...
        var co = d["confidence"];
        // // console.log(linearColorScale_spot(linear(co))) rgb(z,z,z)
        return linearColorScale_spot(linear(co))
      }
    })
    ...
    .attr("fill-opacity",(d)=>{
      if(data_rem_item.findIndex(item=>d.ids === item.ids) !== -1){
        return 1;
      }
      return 0.5;
    })
...
//忘记没加
  const circle=svg.append("g")
      ...
      .attr("stroke",function(d){
        if(d.labels===-1){//20220305
      		return "#C00000";
      	}else{
			     var co=d['confidence'];
           return linearColorScale_spot(linear(co))
           //return colorScale_spot(co)
      	}
      })
      .style('stroke-width', 0.5)
      //.style('fill-opacity', 0.2)

"""
Graph Kernel GED Similarity Calculation and Clustering

Author: Your Name
Date: 12/15/2023

This file contains Python code to calculate graph edit distance (GED) similarity matrices
for multiple graph datasets and perform clustering on the GED matrices.
The code first loads multiple graph datasets and calculates all-pairs GED values
between graphs in each dataset using the Bipartite GED algorithm. The GED values
are stored in similarity matrices for each dataset.
Clustering is then performed on the GED matrices using the t-SNE dimensionality
reduction algorithm followed by DBSCAN clustering. The clustering results are compared
to the true labels for each graph to evaluate clustering performance.

The main steps are:
1. Calculate GED similarity matrices for each dataset
2. Perform t-SNE + DBSCAN clustering on each GED matrix
3. Compare clusters to true labels

Input:
- Multiple graph datasets, each containing graphs and their true labels
Output:
- GED similarity matrices for each input graph dataset
- Clustering results (cluster labels) for graphs in each dataset
- Evaluation of clustering performance by comparing clusters to true labels

This provides a complete workflow for evaluating graph kernel similarity measures
and clustering based on those similarities.
"""
...
def get_matrix():
    ...
    for i in range(0,4):
        ...
        write_json(ged_matrix.tolist(), './data/G_Graph_Data/edit2/matrix_{}.json'.format(ds_name))
...
def clustering():
    ...
    for di in range(len(ds_name_list)):
        ...
        matrix_x = read_json('./data/G_Graph_Data/edit/matrix_{}.json'.format(ds_name_list[di]))
        uuid_list = []
